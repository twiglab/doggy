...
func (s *Schemaless) handleCount(_ context.Context, data human.DataMix) error {
	...
	enc.AddTag(TAG_CODE, data.Head.Code)
	enc.AddTag(TAG_PROJECT, data.Head.Project)
	enc.AddTag(TAG_SN, data.Head.SN)
	enc.AddTag(TAG_UUID, data.Head.ChannelID)
	enc.AddTag(TAG_X, data.Head.X)
	enc.AddTag(TAG_Y, data.Head.Y)
	enc.AddTag(TAG_Z, data.Head.Z)
